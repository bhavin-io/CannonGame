...
public class CannonView extends SurfaceView
   ...
   public static class GameOverDialogFragment extends DialogFragment {
      ...
      private static final String ARG_SHOTS_FIRED = "shots_fired";
      private static final String ARG_TOTAL_TIME = "total_time";

      public static GameOverDialogFragment newInstance(int messageId, int shotsFired, double totalTime,CannonView cannonView) {
         GameOverDialogFragment frag = new GameOverDialogFragment();
         frag.cannonView = cannonView; // Store the reference to CannonView
         Bundle args = new Bundle();
         args.putInt(ARG_MESSAGE_ID, messageId);
         args.putInt(ARG_SHOTS_FIRED, shotsFired);
         args.putDouble(ARG_TOTAL_TIME, totalTime);
      ...
      public Dialog onCreateDialog(Bundle savedInstanceState) {
         ...
         int shotsFired = getArguments().getInt(ARG_SHOTS_FIRED);
         double totalTime = getArguments().getDouble(ARG_TOTAL_TIME);

         AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
         builder.setTitle(getResources().getString(messageId));
         builder.setMessage(getResources().getString(R.string.results_format, shotsFired, totalTime));
         builder.setPositiveButton(R.string.reset_game,
                 new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                       // Implement action on reset game
                       cannonView.dialogIsDisplayed = false;
                       cannonView.newGame(); // set up and start a new game
                    }
                 }
         );
   ...
   private void showGameOverDialog(final int messageId) {
      // Use the activity to run on the UI thread
      activity.runOnUiThread(new Runnable() {
         @Override
         public void run() {
            GameOverDialogFragment dialogFragment = GameOverDialogFragment.newInstance(messageId, shotsFired, totalElapsedTime, CannonView.this
            );
            dialogFragment.show(activity.getFragmentManager(), "game_over");
         }
      });
   }
